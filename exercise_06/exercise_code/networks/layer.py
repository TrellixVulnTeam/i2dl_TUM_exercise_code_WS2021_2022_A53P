...
class LeakyRelu:
    ...
    def forward(self, x):
        ...
        ########################################################################
        outputs = np.maximum(x,0) + np.minimum(x,0) * self.slope
        cache = (x>0) # return index where x>0
    ...
    def backward(self, dout, cache):
        ...
        dx = dout * np.where(cache>0, cache, self.slope) # index >0 return 1 otherwise return slope
